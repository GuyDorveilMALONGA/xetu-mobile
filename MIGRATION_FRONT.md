...
| 2026-06-29 | Stabilisation Codex | Reprise directe après retours utilisateur : suppression de l'erreur visible/bouton `Réessayer` quand aucun bus n'est actif, zoom carte compact, tab-bar forcée à 72px réels sans halo actif, espaces bas Chat/Itinéraire/Mes lignes corrigés, modal abonnement auto-height au-dessus de la nav avec bouton `Fermer`, fallback local Itinéraire depuis `xetu_mvp.json` si `/api/route` échoue, fallback `navigator.geolocation` derrière Capacitor dans Signalement. | `npm.cmd run build` succès ; `npm.cmd test -- --watch=false --browsers=ChromeHeadless --progress=false` → **121/121 SUCCESS** ; preview 393x852 vérifié : Carte sans `Impossible...`/`Réessayer`, Chat input bottom 756 vs nav y 772, Itinéraire fréquent → résultat local, modal abonnement bottom 766 vs nav y 772. |
